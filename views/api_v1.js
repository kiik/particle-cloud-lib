    ...
    req.busboy.on('file', function(fieldname, file, filename, encoding, mimetype) {
      file.on('data', function(data) {
        ...
        if(!req.files.hasOwnProperty(name)) {
          ...
          req.files[name].data = Buffer.concat([req.files[name].data, data]);
          req.files[name].length = req.files[name].data.length;
        }
...
var Api = {
  loadViews: function (app) {
    ...
    app.put('/v1/devices/:coreid', FileHandler, Api.set_core_attributes);
  ...
  fn_call: function (req, res) {
    ...
    when(coreResult)
      ...
      function (arr) {
        ...
        try {
          //logger.log("FunCall - heard back ", { coreID: coreID, userId: userId.toString() });
  ...
  flash_core_dfd: function (req) {
    ...
    if (req.files) {
      args.data = req.files.file.data; //fs.readFileSync(req.files.file.path);
    }
